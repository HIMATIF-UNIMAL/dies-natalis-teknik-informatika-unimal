...
class Vocal_solo extends CI_Controller {
  ...
  function __construct(){
    ...
    ');
    redirect(base_url('vocal_solo/setting')); 
	}

	public function aksi_tolak($id){
		$this->db->set('status', 2);
		$this->db->where('id', $id);
    $this->db->update('tbl_vocal_solo');
    $this->session->set_flashdata('msg', '
    <div class="position-fixed" style="z-index: 9999999">
      <div id="toast" class="bs-toast toast toast-placement-ex m-2 fade bg-success top-0 start-50 translate-middle-x show" role="alert" aria-live="assertive" aria-atomic="true">
        <div class="toast-header">
    ...
    ');
    redirect(base_url('vocal_solo/pending')); 
	}
	...
	public function pending()
	{
		$data['title'] = 'Pending';
		$data['setting'] = $this->db->get_where('tbl_setting', ['id' => 4])->row_array();
		$data['hasil'] = $this->db->get_where('tbl_vocal_solo', array('status'=> 0))->result();
		$this->load->view('admin/header', $data);
		$this->load->view('admin/vocal_solo/pending');
		$this->load->view('admin/footer');
	}
  ...
  public function aktif()
	{
		$data['title'] = 'Aktif';
		$data['setting'] = $this->db->get_where('tbl_setting', ['id' => 4])->row_array();
		$data['hasil'] = $this->db->get_where('tbl_vocal_solo', array('status'=> 1))->result();
		$this->load->view('admin/header', $data);
		$this->load->view('admin/vocal_solo/aktif');
		$this->load->view('admin/footer');
	}
  ...
  public function pdf()
	{
		$data['hasil'] = $this->db->get_where('tbl_vocal_solo', array('status'=> 1))->result();
		$mpdf = new \Mpdf\Mpdf();
		$html = $this->load->view('admin/vocal_solo/print',$data,true);
		$mpdf->WriteHTML($html);
		//$mpdf->Output(); // opens in browser
		$mpdf->Output('Absensi-vocal_Solo.pdf','D'); // it downloads the file into the user system, with give name
	}
	...
	public function tolak()
	{
		$data['title'] = 'Tolak';
		$data['setting'] = $this->db->get_where('tbl_setting', ['id' => 4])->row_array();
		$data['hasil'] = $this->db->get_where('tbl_vocal_solo', array('status'=> 2))->result();
		$this->load->view('admin/header', $data);
		$this->load->view('admin/vocal_solo/tolak');
		$this->load->view('admin/footer');
	}

	private function _sendEmail($id, $type)
  {
      $user = $this->db->get_where('tbl_vocal_solo', ['id' => $id])->row();
      $setting = $this->db->get_where('tbl_setting', ['id' => 4])->row();
